// scripts/paystackSubaccounts.js
// Bulk Paystack subaccount creation script for Blorb marketplace
...
// Initialize Firebase Admin with service account credentials (server-side, bypasses security rules)
let appInitialized = false;
try {
  const serviceAccount = require('../serviceAccountKey.json');
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
    projectId: serviceAccount.project_id || 'blorbmart2'
  });
  appInitialized = true;
} catch (e) {
  ...
  try {
    ...
    console.error('❌ Failed to initialize Firebase Admin. Provide serviceAccountKey.json under scripts/.. or set GOOGLE_APPLICATION_CREDENTIALS.');
    throw inner;
  }
}
...
// Configuration
// Sanitize secret key: trim whitespace and surrounding quotes
const PAYSTACK_SECRET_KEY_RAW = process.env.PAYSTACK_SECRET_KEY;
const PAYSTACK_SECRET_KEY = PAYSTACK_SECRET_KEY_RAW
  ? PAYSTACK_SECRET_KEY_RAW.trim().replace(/^['\"]|['\"]$/g, '')
  : undefined;
...
// New: make collection and role filter configurable
const USERS_COLLECTION = process.env.USERS_COLLECTION || 'users';
const ROLE_FIELD = process.env.ROLE_FIELD || 'role';
const ROLE_VALUES = (process.env.ROLE_VALUES || 'seller')
  .split(',')
  .map(v => v.trim())
  .filter(Boolean);
const BYPASS_ROLE_FILTER = String(process.env.BYPASS_ROLE_FILTER || 'false').toLowerCase() === 'true';
...
if (!PAYSTACK_SECRET_KEY.startsWith('sk_')) {
  console.error('❌ Invalid PAYSTACK_SECRET_KEY: must start with sk_ (got a different prefix, e.g., pk_)');
  process.exit(1);
}
...
async function fetchSellers() {
  try {
    // Log connected project and query plan
    const connectedProjectId = admin.app().options.projectId;
    log(`Fetching sellers from Firestore... (project: ${connectedProjectId}, collection: ${USERS_COLLECTION}, ${ROLE_FIELD} in [${ROLE_VALUES.join(', ')}])`);
    
    // Build query
    let queryRef = db.collection(USERS_COLLECTION);
    if (!BYPASS_ROLE_FILTER) {
      // Use an 'in' query to match multiple role values (case or synonyms)
      queryRef = queryRef.where(ROLE_FIELD, 'in', ROLE_VALUES);
    } else {
      log(`Bypassing role filter; fetching all documents from '${USERS_COLLECTION}'`);
    }
    const sellersSnapshot = await queryRef.get();
    ...
    // If none found, run a brief probe to help diagnose collection/field naming
    if (sellers.length === 0) {
      try {
        const probeSnap = await db.collection(USERS_COLLECTION).limit(3).get();
        if (probeSnap.empty) {
          log(`Probe: collection '${USERS_COLLECTION}' has 0 documents`);
        } else {
          let probeCount = 0;
          probeSnap.forEach(doc => {
            probeCount++;
            const data = doc.data();
            const keys = Object.keys(data).slice(0, 10);
            log(`Probe doc ${doc.id} keys: ${keys.join(', ')}`);
          });
          log(`Probe: sampled ${probeCount} docs from '${USERS_COLLECTION}'`);
        }
      } catch (probeErr) {
        log(`Probe error on collection '${USERS_COLLECTION}': ${probeErr.message}`, 'error');
      }
    }
...
async function createPaystackSubaccount(seller, retryCount = 0) {
  try {
    ...
    // Flexible fallbacks for seller profile fields
    const businessNameOrStore = seller.businessName || seller.storeName || seller.fullName || seller.name || `Seller ${seller.id}`;
    const contactName = seller.fullName || seller.name || seller.businessName || seller.storeName || businessNameOrStore;
    const contactPhone = seller.phoneNumber || seller.phone || '';

    const payload = {
      business_name: businessNameOrStore,
      settlement_bank: bankCode,
      account_number: bankDetails.accountNumber,
      percentage_charge: PLATFORM_PERCENTAGE,
      description: `Blorb Marketplace - ${seller.fullName || seller.name || seller.storeName || seller.id}`,
...
async function updateSellerDocument(seller, subaccountData) {
  try {
    ...
    await db.collection(USERS_COLLECTION).doc(seller.id).update(updateData);
...
async function main() {
  try {
    log('🚀 Starting bulk Paystack subaccount creation script...');
    log(`Platform fee: ${PLATFORM_PERCENTAGE}%`);
    log(`Batch size: ${BATCH_SIZE}`);
    log(`Retry attempts: ${RETRY_ATTEMPTS}`);
    // Masked key diagnostics
    const masked = PAYSTACK_SECRET_KEY
      ? PAYSTACK_SECRET_KEY.slice(0, 6) + '...' + PAYSTACK_SECRET_KEY.slice(-2)
      : 'undefined';
    log(`Using Paystack key: ${masked}`);
    ...
    // For automated execution, you can comment out this confirmation
