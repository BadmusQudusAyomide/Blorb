...
const FinancesPage = () => {
    ...
    try {
      ...
      const ordersData = ordersSnapshot.docs.map((doc) => {
        ...
        return {
          ...
          sellerStatuses: data.sellerStatuses || {},
        } as Order;
      });
  ...
  const preparePayoutData = () => {
    ...
    return payouts
      ...
      .reduce((acc: Record<string, number>, payout) => {
        const monthYear = payout.createdAt
          .toDate()
  ...
  // Prepare chart data
  const revenueChartData = Object.entries(prepareRevenueData()).map(
    ([date, revenue]) => ({
      date,
      revenue,
    })
  ...
  const payoutChartData = Object.entries(preparePayoutData()).map(
    ([month, amount]) => ({
      month,
      amount,
    })
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="bg-white rounded-lg shadow border border-gray-200 p-4">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Transfer your available balance to your bank account. Minimum
                withdrawal: ₦1,000
              </p>
              <button
